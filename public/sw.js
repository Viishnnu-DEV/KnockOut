...
self.addEventListener('fetch', (event) => {
  ...
  /* Static assets (Fonts/Images) — Cache first, fallback to network */
  if (
  ...
  /* Styles and Scripts — Network first, fallback to cache */
  if (
...
async function checkAndFireNotifications() {
  try {
    ...
    const starredMatches = await getFromIDB('starred_matches') || [];
    if (!starredTeams.length && !starredMatches.length) return;
    ...
    const upcomingMatches = matches.filter((match) => {
      ...
      const isStarredTeam =
        starredTeams.includes(match.teamA) ||
        starredTeams.includes(match.teamB) ||
      ...
      const isStarredMatch = starredMatches.includes(match.id);
      const notAlreadyNotified = !match._notified;
      
      return isInWindow && (isStarredTeam || isStarredMatch) && notAlreadyNotified;
    });
...
self.addEventListener('message', (event) => {
  ...
  switch (type) {
    ...
    case 'UPDATE_STARRED_MATCHES':
      saveToIDB('starred_matches', data.matches)
        .then(() => {
          console.log('[SW] Starred matches updated:', data.matches);
          event.source?.postMessage({ type: 'STARRED_MATCHES_UPDATED' });
        });
      break;

    case 'SCHEDULE_NOTIFICATIONS':
      console.log('[SW] Notification check requested by app');
      checkAndFireNotifications()
        .then(() => event.source?.postMessage({ type: 'NOTIFICATIONS_CHECKED' }))
        .catch((err) => console.error('[SW] Notification check failed:', err));
      break;

    case 'SCHEDULE_REMINDERS':
      handleScheduleReminders(data.reminders);
      break;
...
const scheduledAlerts = new Map(); // matchId → timeoutId

function handleScheduleReminders(reminders) {
  if (!Array.isArray(reminders)) return;

  const now = Date.now();

  // Clear old timers
  scheduledAlerts.forEach(id => clearTimeout(id));
  ...
  reminders.forEach(reminder => {
    ...
    // Skip past or too-far-future (> 7 days)
    ...
    scheduledAlerts.set(reminder.matchId, timerId);
    console.log(
      `[SW] Reminder scheduled: ${reminder.homeTeam} vs ${reminder.awayTeam} in ${Math.round(delay/60000)} min`
    );
  });

  console.log(`[SW] ${scheduledAlerts.size} reminders active`);
}
