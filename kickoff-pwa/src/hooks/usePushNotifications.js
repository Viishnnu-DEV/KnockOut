...
export function usePushNotifications() {
  ...
  const scheduleMatchAlerts = useCallback(
    async (matches, starredTeams, teamMap = {}) => {
      ...
      for (const match of matches) {
        ...
        const homeId = match.home_team_id || match.home_team?.name;
        const awayId = match.away_team_id || match.away_team?.name;
        const isStarred =
          starredTeams.includes(homeId) || starredTeams.includes(awayId);
  ...
  const showLocalNotification = async ({ title, body, matchId, teamA, teamB, timeIST }) => {
