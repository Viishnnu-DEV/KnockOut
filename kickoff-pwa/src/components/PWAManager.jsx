...
export default function PWAManager({ matches = [], starredTeams = [], teamMap = {} }) {
  const {
    isInstallable,
    isInstalled,
    isOffline,
  ...
  useEffect(() => {
    if (!isInstallable || installDismissed || isInstalled) return;
  ...
  /* Show update toast */
  useEffect(() => {
    if (updateAvailable) setShowUpdateToast(true);
  }, [updateAvailable]);

  /* Show notification prompt after install or after 60s */
  useEffect(() => {
    if (permission === 'default' && pushSupported && !showNotifPrompt) {
      const timer = setTimeout(() => setShowNotifPrompt(true), 60000);
      return () => clearTimeout(timer);
    }
  ...
  useEffect(() => {
    if (permission === 'granted' && matches.length && starredTeams.length) {
      scheduleMatchAlerts(matches, starredTeams, teamMap);
    }
  }, [matches, starredTeams, permission]);
  ...
  const handleEnableNotifications = async () => {
    ...
    if (result === 'granted' && matches.length) {
      const count = await scheduleMatchAlerts(matches, starredTeams, teamMap);
      if (count > 0) {
  ...
  return (
    <>
      ...
      {showNotifPrompt && permission === 'default' && pushSupported && (
        <div style={{
          position: 'fixed',
